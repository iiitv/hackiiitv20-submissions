...
session_start();
 ...
 if(isset($_SESSION['login_user_connect']) && isset($_REQUEST['name'])){
	...
	$dts = explode("&",base64_decode($_REQUEST['name']));
	$u_id = $dts[0];
	
	if($u_id == $id){
		
		header("Location:userProfile.php?name=".$_REQUEST['name']);
...
<?php
	if($u_id != $id){
		...
		$x = false;
		$qry = "SELECT * FROM friendrequest WHERE sender_id = $id AND receiver_id = $u_id";
...
?>
					<div class="flex justify-center custom-button">
              
<?php
	if($x){
?>
<button class="w3-button w3-blue w3-margin-right" style="transition-duration:0.2s" >
...
?>
<button class="w3-button w3-blue w3-margin-right" onclick="sendRequest(<?php echo $u_id ?>)" style="transition-duration:0.2s" >
...
?>
                        ...
                        <button
                            class="w3-button w3-green w3-margin-left" style="transition-duration:0.2s"><i class="fa fa-mail-forward"></i> Send message</button>
                    </div>
<?php
	}
?>
